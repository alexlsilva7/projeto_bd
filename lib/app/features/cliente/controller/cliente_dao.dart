...
class ClienteDao {
  static Future<List<Cliente>> getClientes() async {
    ...
    for (int i = 0; i < clientes.length; i++) {
      final telefones = await conn.query(
          'SELECT telefone FROM TelefoneCliente WHERE clienteId = ?',
          [clientes[i].id]);
      final emails = await conn.query(
          'SELECT email FROM EmailCliente WHERE clienteId = ?',
          [clientes[i].id]);
      //add telefones and emails
      clientes[i].telefones = telefones
          .map((row) => TelefoneCliente(
                clienteId: clientes[i].id!,
                telefone: row['telefone'],
              ))
          .toList();
      clientes[i].emails = emails
  ...
  static Future<Cliente?> getCliente(int id) async {
    ...
    final cliente = clientes.isEmpty ? null : clientes.first;
    return cliente;
  }
